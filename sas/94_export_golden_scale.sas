/*******************************************************************************/
/*  SAS Program: 94_export_golden_scale.sas                                   */
/*  Purpose: Export the large-scale golden set for the synthetic scale         */
/*  fixture. Run after 95_load_mock_scale.sas, 02_data_cleaning.sas,           */
/*  03_aggregation_reporting.sas and 04_risk_segmentation.sas in the same      */
/*  session, so the work library holds home_equity, home_equity_clean,         */
/*  home_equity_imputed, home_equity_filtered, home_equity_final and           */
/*  home_equity_risk built from data/home_equity_scale.csv.                    */
/*                                                                             */
/*  Reproduces, for the scale fixture, the full aggregate set that             */
/*  99_export_golden.sas exports for production. Copy the CSVs it writes       */
/*  into tests/parity/golden/scale/.                                           */
/*******************************************************************************/

%let outdir = /data/golden/scale;

/* Row counts at every stage of the pipeline */
proc sql;
    create table work.row_counts as
        ...
        union all
        select 'home_equity_imputed'  as STAGE length=20, count(*) as N from work.home_equity_imputed
        union all
        select 'home_equity_filtered' as STAGE length=20, count(*) as N from work.home_equity_filtered
        union all
...
/* JOB and REASON carry missing values by construction, so these tables pin   */
/* the missing-class behaviour at scale.                                      */
proc freq data=work.home_equity_final noprint;
...
/* Two-way segment x outcome counts */
proc freq data=work.home_equity_risk noprint;
...
/* Distribution shape of every continuous column (SAS PCTLDEF=5) */
proc means data=work.home_equity_final noprint;
...
/* Keep the formatted view for fidelity, export the format-stripped dataset   */
/* as the file the parity tests compare against (same as 99).                 */

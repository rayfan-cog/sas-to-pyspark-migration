...
| `freq_job.csv` | `JOB`, `FREQUENCY`, `PERCENT` | yes — exact counts, percent to 1e-9. Includes PROC FREQ's missing-class row (blank `JOB`, blank `PERCENT`), which the test compares against the null count instead (D-016) |
